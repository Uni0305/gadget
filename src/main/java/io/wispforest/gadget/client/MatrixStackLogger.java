...
public final class MatrixStackLogger {
    ...
    public static void logOp(MatrixStack stack, boolean push, int indent) {
        if (ERROR_MODE != 2 && !(Gadget.CONFIG.internalSettings.debugMatrixStackDebugging() && InputUtil.isKeyPressed(MinecraftClient.getInstance().getWindow(), GLFW.GLFW_KEY_LEFT_SHIFT))) return;
    ...
    public static void startLoggingIfNeeded() {
        if (Gadget.CONFIG.internalSettings.debugMatrixStackDebugging() && InputUtil.isKeyPressed(MinecraftClient.getInstance().getWindow(), GLFW.GLFW_KEY_LEFT_SHIFT)) {
